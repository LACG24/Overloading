...
int main() {
  ...
  vector <Video*> videos;
  cout << "     Welcome!     " <<  "\n********Menu********\n1. Load data file \n" <<
  "2. Show videos in general with a certain review or of a certain genre\n" <<
  "3. Show episodes of a certain series with a certain review\n" <<
  "4. Show movies with a certain review\n" <<
  "5. Rate a video\n" << 
  "0. Exit\n"; 
  do{
  cout << "Option: "; 
  ...
  switch(option){
    ...
    case 2:{ 
      if (videos.size() == 0){
        ...
        cout << "Choose by genre or review (g/r): ";
        cin >> secondOption;
        while(secondOption != 'g' && secondOption != 'r'){
          cout << "Incorrect option\nChoose by genre or review (g/r): ";
          cin >> secondOption;
        }
        if (secondOption == 'g'){
          cout << "Genre to see: Drama, Action, Mystery (D, A, M): ";
          cin >> secondOption;
          while(secondOption != 'D' && secondOption != 'A' && secondOption != 'M'){
            cout << "Incorrect option\ngenre to see: Drama, Action, Mystery (D, A, M): ";
            cin >> secondOption;
          }
          for (Video* video: videos){
            if (secondOption == 'D' && video->getGenre() == "Drama"){
              cout << *video;
            }
            else if (secondOption == 'A' && video->getGenre() == "Action"){
              cout << *video;
            }
            else if (secondOption == 'M' && video->getGenre() == "Mystery"){
              cout << *video;
            }
            else {
              continue;
